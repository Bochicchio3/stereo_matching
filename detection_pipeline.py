...
cv2.namedWindow(state.WIN_NAME, 1500)
...
def update( imgL, imgR, sliderValue = 0):
    ...
    disparity = stereo.compute(
        imgL, imgR).astype(np.uint16) / 16.0
...
infrared_right=np.zeros((h,w), dtype=np.uint8)
...
while True:
    ...
    if not state.paused:
        ...
        infrared_left=np.asanyarray(infrared_left_frame.get_data())
        infrared_right=np.asanyarray(infrared_right_frame.get_data())

        
        disparity=update(infrared_left, infrared_right)
        # depth_map_SGMB=depth_instrinsics.fx*0.055/disparity
        depth_map_SGBM=0.055*depth_intrinsics.fx/disparity
        depth_map_SGBM[depth_map_SGBM==math.inf]=0
        depth_map_SGBM_gradient= cv2.applyColorMap(cv2.convertScaleAbs(disparity, alpha=0.03), cv2.COLORMAP_JET)
        depth_colormap_gradient = cv2.applyColorMap(cv2.convertScaleAbs(depth_image, alpha=0.03), cv2.COLORMAP_JET)

        # Stack both images horizontally
        images = np.hstack((color_image, depth_colormap_gradient))
        infrared_images = np.hstack((infrared_left,infrared_right))
...
# ---------------------------------------------------------------------------- #
    ...
    # cv2.namedWindow('ReaSense2', cv2.WINDOW_AUTOSIZE)
    # cv2.namedWindow('Infrared_frames', cv2.WINDOW_AUTOSIZE)

    cv2.imshow('RealSense2', images)
    cv2.imshow(state.WIN_NAME, out) 
    cv2.imshow('Infrared_frames', infrared_images)
    
    
    # cv2.imshow('Left', imgL)
    # cv2.imshow('Right', imgR)
    cv2.imshow('Disparity',depth_map_SGBM_gradient)
